...
def generate_price_chart_image(prices, selected_grade="PSA10", y_tick_font_size=26):
    ...
    for item in reversed(prices or []):
        ...
        date_text = short_date_text(raw_date)
    ...
    fig, ax = plt.subplots(figsize=(10.4, 5.1), dpi=200)
    ...
    if valid_items:
        ...
        if len(date_tick_positions) > max_tick_count:
            ...
            for i in range(max_tick_count):
                selected_index = round(i * (len(date_tick_positions) - 1) / (max_tick_count - 1))
        ...
        ax.tick_params(axis="x", length=0, pad=9, colors="#777777")
        ...
        ax.tick_params(axis="y", labelsize=y_tick_font_size, length=0, colors="#666666")
        ...
        ax.text(0.5, 0.5, f"查無 {selected_grade} 成交資料", ha="center", va="center", fontsize=20, color="#777777")
        ax.set_xticks([])
        ax.set_yticks([])
        for spine in ax.spines.values():
            spine.set_visible(False)

    fig.subplots_adjust(left=0.15, bottom=0.22, right=0.98, top=0.90)
...
def generate_market_card_image(product, prices, selected_grade="PSA10", jpy_rate=None):
    ...
    draw.rounded_rectangle(outer_box, radius=18, fill="white")

    # 左側商品圖
    left_box = (58, 88, 500, 690)
    draw.rounded_rectangle(left_box, radius=12, fill="#FAFAFA")
    ...
    if product_image:
        ...
        product_image = product_image.resize((new_w, new_h), resample_filter)
        ...
        draw.text((left_box[0] + 120, left_box[1] + 250), "No Image", fill="#999999", font=placeholder_font)
    ...
    while text_width(draw, product_name, title_font_dynamic) > right_w and title_font_size > 38:
        title_font_size -= 2
        title_font_dynamic = get_font(title_font_size, bold=True)

    draw.text((title_x, title_y), product_name, fill="#222222", font=title_font_dynamic)
    ...
    chart_image = create_contain_image(chart_image, (930, 390), bg_color=(255, 255, 255))
    card.paste(chart_image, (right_x + 6, 205))

    # 下方統計區（放大並重新對齊）
    bottom_stat_y = 690
    grade_center_y = bottom_stat_y + 58
    grade_text_bbox = draw.textbbox((0, 0), selected_grade, font=grade_font)
    grade_h = grade_text_bbox[3] - grade_text_bbox[1]
    draw.text((90, grade_center_y - grade_h / 2), selected_grade, fill="#2F5FE8", font=grade_font)
    ...
    for idx, (label, value) in enumerate(stat_items):
        ...
        draw.text((x1 + (stat_w - jpy_w) / 2, y1 + 34), jpy_text, fill="#222222", font=stat_jpy_font)

        twd_text = format_twd_text(value, jpy_rate)
        twd_w = text_width(draw, twd_text, stat_twd_font)
        draw.text((x1 + (stat_w - twd_w) / 2, y1 + 88), twd_text, fill="#999999", font=stat_twd_font)


    rate_text = f"台灣銀行日圓即期匯率：{jpy_rate}" if jpy_rate else "台灣銀行日圓即期匯率：取得失敗"
    ...
    draw.text((footer_right_x - rate_w, footer_y_1), rate_text, fill=footer_color, font=footer_font)
    draw.text((footer_right_x - source_w, footer_y_2), source_text, fill=footer_color, font=footer_font)
